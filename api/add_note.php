...
try {
    ...
    if (empty($note)) { echo json_encode(['error' => 'Catatan tidak boleh kosong']); exit; }
    ...
    $stmt = $pdo->prepare("INSERT INTO task_notes (task_id, user_id, note) VALUES (?, ?, ?)");
    $stmt->execute([$taskId, $userId, $note]);

    $noteId = (int)$pdo->lastInsertId();
    $fullName = $_SESSION['full_name'] ?? $_SESSION['username'] ?? 'Unknown';

    try { $pdo->prepare("INSERT INTO activity_log (user_id, task_id, action, description) VALUES (?,?,?,?)")->execute([$userId, $taskId, 'updated', 'Menambah catatan']); } catch (Throwable $e) {}
