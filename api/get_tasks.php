            ...
            if (!empty($task['notes_json'])) {
                ...
                if (is_array($notes)) {
                    ...
                    foreach ($notes as $i => $note) {
                        ...
                        $hidden = $i < $lastIdx ? ' style="display:none;"' : '';
                        $notesHtml .= '<div class="note-item"' . $hidden . '><div class="note-text">' . $noteText . '</div><div class="note-meta">' . $noteUser . ' &middot; ' . $noteTime . '</div></div>';
                    }
                    if ($noteCount > 1) {
                        $notesHtml .= '<button class="notes-toggle" onclick="togglePrevNotes(' . $tid . ')" style="font-size:12px;padding:4px 8px;" id="prevNotesBtn-' . $tid . '">📋 Lihat ' . ($noteCount - 1) . ' komentar sebelumnya</button>';
                    }
                }
            }
