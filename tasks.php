...
.notes-toggle { display:flex;align-items:center;gap:6px;padding:6px 10px;border-radius:6px;font-size:12px;font-weight:500;color:var(--gray-500);cursor:pointer;background:transparent;border:none;transition:all 0.2s;margin:4px 0;width:fit-content; }
.notes-toggle:hover { background:var(--gray-100);color:var(--primary); }
.notes-toggle .chevron { display:inline-block;transition:transform 0.25s ease;font-size:10px; }
.notes-toggle .chevron.open { transform:rotate(90deg); }
</style>
<script>
const state = {
    status: '',
...
function togglePrevNotes(id) {
    const container = document.getElementById('notes-' + id);
    const items = container.querySelectorAll('.note-item');
    const btn = document.getElementById('prevNotesBtn-' + id);
    const lastIdx = items.length - 1;
    const allHidden = lastIdx > 0 && items[0].style.display === 'none';
    for (let i = 0; i < lastIdx; i++) {
        items[i].style.display = allHidden ? 'flex' : 'none';
    }
    if (btn) {
        btn.textContent = allHidden ? '🙈 Sembunyikan' : '📋 Lihat ' + lastIdx + ' komentar sebelumnya';
    }
}
